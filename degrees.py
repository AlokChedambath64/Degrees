...
def shortest_path(source, target):
    ...
    #initailize the initial state
    initial = Node(source, None, None)
    #variable to track how many nodes we have traversed
    trav = 0
    #add to frontier
    frontier = StackFrontier()
    #adding initial node to the frontier 
    frontier.add(initial)
    #to keep track of the final answer
    path = []
    #to keep track of the explored nodes
    explored = set()
    #while loop to traverse through the data till frontier is empty
    while (frontier.empty != 0):
        ...
        for movie, actor in neighbors:
            if actor not in explored and not frontier.contains_state(actor):
                ...
                #if the target has been found return the target
